...
from algorithm import image_encryption
...
def open_file():
    ...
    if name:
        file_name = image_encryption.search_file_name(name, extension=True)
...
def message(name, button, mgs_label):
    ...
    if button["text"] == "Encryption":
        mgs = image_encryption.image_encryption(name)
        mgs_label.config(text=mgs)

    if button["text"] == "Decryption":
        mgs = image_encryption.image_decryption(name)
        mgs_label.config(text=mgs)
...
ttk.Button(root, text="Chose File", command=open_file).pack()
